...
| §12.4 | `ProtectedRoute` shows a spinner while auth state is `initializing`, calls `GET /api/v1/auth/me` on mount, clears auth state and redirects on failure, and redirects unauthenticated users with `<Navigate to="/login" state={{ from: location }}>`; `PublicRoute` is the inverse guard and redirects authenticated users to `/dashboard` except on `/` (Landing), which authenticated users may view (`/login`, `/register`, and any future public pages redirect to `/dashboard`) | Routing Layout (3), Requirements (REQ-095), User Stories (US-028) |
...
| §12.3 | GlobalSearchDialog: full-screen below 600px / 768px landscape (no border radius, 100vh); centered 600px wide / 80vh tall on all larger screens (≥600px); closed by back arrow, Escape, or outside click; RHF `register('search')`, fires on Enter or click, no debounce; results grouped by entity (Reports, Branches) in MuiAccordion; empty state "No results found" | MUI Component Standards (5), Requirements (REQ-099) |
...
| §14 (1.1–1.14) | Component catalog: MuiAppbar (file `client/src/components/reusable/MuiAppbar.jsx`, props `position`/`elevation`/`color`/`sx` defaults, left logo → `/dashboard` if authenticated else `/`, right section conditional on auth, PublicLayout vs AppShell behaviors, avatar 32px below 600px / 36px at or above 600px, auth detection via Redux `authSlice` `useSelector`, exclusions — search dialog, user dropdown, hamburger); MuiButton; MuiDialog (title bottom divider, scrollable content capped at `maxHeight: 400` with `p: 0` — the retired `contentSx` prop is gone; callers apply the paper treatment `sx={{ "& .MuiPaper-root": { m: 1, py: 1 } }}` plus an inner inset (`px: 1, py: 2` grid / `p: 1` list); actions divider; responsive fullscreen down('sm') OR down('md')+landscape, overridable via explicit `fullScreen` — filter, create report, branch form, and branch selector dialogs pass `fullScreen={false}`); MuiTextField; MuiSelect; MuiDatePicker (Ethiopian calendar — `client/src/utils/ethiopianDate.js` with `ethiopianToGregorian`/`gregorianToEthiopian`, custom lightweight conversion no npm package, DD-MM-YY display e.g. `25-02-18`, English day names, English month names mapped to Ethiopian months September…August + Pagume, RHF via Controller, `LocalizationProvider` + `AdapterDayjs` in `main.jsx`); MuiPagination (count = server `totalPages` from `mongoose-paginate-v2`, constants `PAGINATION_DEFAULT_PAGE=1`/`PAGINATION_DEFAULT_LIMIT=10`/`PAGINATION_MAX_LIMIT=100`); MuiDataGrid (columns in `client/src/components/columns/*.jsx` action column last, View/Edit/Archive/Delete icon colors via `sx`, archive→MuiConfirmDialog→restore or delete flow, `checkboxSelection` on all viewports + `disableRowSelectionOnClick` + export button, toolbar via `showToolbar` (v9 default: Columns toggle, Filter with count badge, CSV export; `showQuickFilter: false` + wrapping row; `csvOptions.getRowsToExport` — the v9 key — exports selected rows when a selection exists), plain function component without `forwardRef`, `paginationMode="server"`, `pageSizeOptions={[10, 25, 50, 100]}`, skeleton via `slotProps.loadingOverlay`, MuiEmptyState `noRowsOverlay` via `emptyResource`, default height `{ xs: 320, sm: 400 }`); MuiConfirmDialog; LoadingSpinner (size default 40, minHeight default `"100vh"`); GlobalSearchDialog (`useForm({ mode: 'onSubmit' })`, uncontrolled `register('search')`, ArrowBackIcon start adornment clears/resets/closes); MuiPageHeader (title + subtitle hidden on vw < 600 portrait, children right slot, `mb: 2`, bottom divider); MuiStatusBadge (statuses `draft`/`audio_attached`/`transcribed`/`reviewed`/`completed` → default/warning/info/primary/success; used in Report Details header (3.6)) | MUI Component Standards (1, 4, 5, 9), UI/UX Spec (10, 11), Requirements (REQ-109, REQ-110) |
| §14 (1.14) + §3.6 (Phase 3 corrections) | MuiEmptyState (file `client/src/components/reusable/MuiEmptyState.jsx`, centered flex column, icon in soft circle, headline "No {resource} yet", optional subtitle/action, `dense` variant for grid `noRowsOverlay`, default minHeight `"320px"`; echoes the `## UI/UX Spec` §3.6 empty states) | MUI Component Standards (9.10) |
...
| Source ref | Fact | Recorded in spec section |
|---|---|---|
| §20.1 | Browser MediaRecorder records each clip into a local-state array via a custom hook; the full array submits as the multipart field `clips`; blobs never persisted to Redux/redux-persist/localStorage, with one Phase 4 corrections exception: recorded-but-unsubmitted clips are persisted in an IndexedDB-backed audio draft store (per-user key, `client/src/utils/audioDraftStore.js`, module-memory fallback) so a failed create never forces a re-record — the draft is restored on the next dialog open and cleared on a successful audio upload (user decision, Phase 4); limits max 15 min/clip (`AUDIO_MAX_DURATION_SEC` = 900) and max 50 MB/clip (`AUDIO_MAX_SIZE_BYTES` = 52428800) enforced client-side after recording stops; a clip over 50 MB blocks submit with a warning and asks for a re-record; MIME priority `audio/webm;codecs=opus` → `audio/webm` → `audio/mp4` → browser default | Audio Recording STT (5), Requirements (REQ-139..141) |
...
| REQ-095 | Route guards must gate access: `ProtectedRoute` shows a spinner while auth state is `initializing`, calls `GET /api/v1/auth/me` on mount, clears auth state and redirects on failure, and redirects unauthenticated users with `<Navigate to="/login" state={{ from: location }}>`; `PublicRoute` is the inverse guard and redirects authenticated users to `/dashboard` except on `/` (Landing), which authenticated users may view (`/login`, `/register`, and any future public pages redirect to `/dashboard`). | Both guards exist with the stated behaviors (including the Landing-page exception); login navigates back to `state.from.pathname` after sign-in. | §12.4 |
...
| REQ-140 | Each clip is recorded with the browser MediaRecorder API into a local-state array via a custom hook; the full array is submitted as the multipart field `clips`; audio blobs are never persisted to Redux, redux-persist, or localStorage, with the §20.1 IndexedDB audio-draft exception (Phase 4 corrections, user decision): unsubmitted clips survive dialog close and refresh, and are cleared on a successful upload. | Multiple clips submit together; blobs exist in component state only, except the IndexedDB audio-draft store used for failed-create recovery. | §20.1 |
...
| DM-08 | Required model fields use plain `required: true`; custom validation messages are defined in the express-validator (`withMessage`) and RHF layers — never in model schemas. | §24.2, §11 |
...
| `GET /oauth/google` | Google OAuth start route; `googleOAuth` controller using `getGoogleOAuthUrl()` service; stubbed until `env.OAUTH_GOOGLE_*` credentials are configured | §11; **route naming finalized in Phase 2 (user decision):** `GET /oauth/google` and `GET /oauth/google/callback` mounted at ROOT level (`app.use('/oauth', oauthRoutes)` in `app.js`), not under `/api/v1`; success redirects to `CLIENT_ORIGIN` |
...
- Static page: no data fetching, no Redux; CTAs use `useNavigate()`; PublicRoute lets authenticated users view the Landing page (`/login`, `/register`, and any future public pages still redirect to `/dashboard`); all text ellipsizes on overflow; no horizontal scroll.
...
- Logo/icon centered (`fontSize="large"`, `color="primary"` — MUI props only, no fixed-pixel icon sizes per user decision; `mb: 1`); title `h5` weight 600 centered "Sign In"; subtitle `body2` `text.secondary` centered `mb: 3` "Welcome back! Sign in to continue".
...
- Page Header: left title "Reports" + subtitle "Manage daily supervision reports"; right: FilterIconButton (MuiBadge `badgeContent={activeFilterCount}`, hidden when 0), ToggleButtonGroup (ViewListIcon / ViewGridView), CreateButton (AddIcon) — the Create button collapses to an icon-only IconButton + Tooltip on viewport widths below 600px in portrait (Phase 3 corrections; `## MUI Component Standards` §4).
- Filter Dialog: MuiDialog `maxWidth="sm"`, title "Filter Reports", **centered on all viewports** (explicit `fullScreen={false}`) with the shared paper treatment `sx={{ "& .MuiPaper-root": { m: 1, py: 1 } }}` and the form grid inset `sx={{ px: 1, py: 2 }}` (DialogContent is `p: 0` with a 400px scroll cap — Phase 3 corrections v3, superseding `contentSx`); row 1 — MuiDatePicker (left) + MuiSelectField single branch (right) in `Grid container spacing={2}` with `size={{ xs: 12, sm: 6 }}` per field (stacked below 600px, side by side from 600px up including mobile landscape); both carry ClearIcon end adornments (`slotProps.input.endAdornment`) — clearing resets the field, decrements `activeFilterCount`, updates the badge immediately; row 2 — MuiSwitch label "Archived" with `labelPlacement="start"` (label reads left of the switch); Cancel resets all filters and badge → 0; Apply sets filter state, closes, badge → count of active filters (1–3) (Phase 3 corrections).
- List toggle → cards: `Grid container spacing={2}`; each report a MuiCard; icon-button actions with MuiTooltip — View (VisibilityIcon, primary → `/reports/:id/details`), Edit (EditIcon, primary → opens the report in the Assistant chat (§11), Archive/Restore/Delete conditional: not archived → ArchiveIcon (warning) + MuiConfirmDialog → `PATCH /api/v1/reports/:id/archive`; archived → RestoreIcon (success) + confirm → `PATCH /api/v1/reports/:id/restore`, DeleteIcon (error) + confirm → `DELETE /api/v1/reports/:id`; below the cards MuiPagination (`page`/`count` from server `totalPages`, `onChange` refetches) — rendered only when `totalPages > 1`; while the first page loads (no data yet) the section shows the LoadingSpinner (`minHeight="320px"`), and an empty first page shows the MuiEmptyState ("No reports yet" + subtitle + Create action) (Phase 3 corrections). Exact dialog copy, toasts, the "Archived" indicator, and the archived-filter-only visibility rule per `## Archive Delete Restore Lifecycle` §7.
- Grid toggle → MuiDataGrid: server-side pagination, toolbar, export selection, action column (same behaviors); empty state via `emptyResource="reports"` (MuiEmptyState).
- The Filter Dialog is the exception on this page: it stays centered on all viewports (`fullScreen={false}`) instead of the responsive fullscreen that MuiDialog applies by default below 600px (and below 768px landscape) (`## MUI Component Standards` §5); the Page Header subtitle hides on viewport widths below 600px in portrait (§14 1.12; `## MUI Component Standards` §4).
...
- Concurrent 401s share a single refresh call: `baseQueryWithReauth` serializes via a module-level mutex — exactly one `POST /api/v1/auth/refresh` per 401 burst; all affected requests await it and then retry with the refreshed cookies. Double responses are impossible in any case; React StrictMode is never disabled or worked around (Standing Guidelines, `docs/implementation-log.md`).
...
- **Client identity convention (binding):** the canonical entity id on the client is the Mongo **`_id`** — used for React keys, navigation URLs, mutation args, filter values, RTK tag ids, and `branchId`/`reportId` payloads. `id` is never read from API docs: DM-03 toJSON/toObject transforms delete the `id` virtual, so the API returns `_id` only. The single MUI X bridge is `MuiDataGrid`'s `getRowId` defaulting to `(row) => row._id` (`## MUI Component Standards` §9.6). Future phases must follow this convention.
...
- **Selection-list lazy fetch (Phase 3 corrections):** endpoints that feed dialog option lists (e.g. `useListBranchesQuery` in ReportFilterDialog and BranchSelectorDialog) pass `{ skip: !open }` — the query fires only while the dialog is open. Navigating to a page (e.g. `/reports`) therefore issues exactly the page's own request (single `GET /reports`), and opening the dialog reuses the RTK Query cache for any subsequent open.
...
5. `<Suspense fallback={<LoadingSpinner />}>` (full-page loading feedback while a lazy page chunk loads — Phase 3 corrections; every page is `React.lazy`, so route changes now show a spinner instead of a blank gap)
6. `<Outlet />`
...
- Inverse guard; redirects authenticated users to `/dashboard` except on `/` (Landing), which authenticated users may view.
...
- Mobile icon-only rule (Phase 3 corrections): action buttons in the right slot collapse to icon-only (raw IconButton + MuiTooltip label, §9.1 icon rules) on viewport widths below 600px in portrait — the same `(max-width:599.95px) and (orientation: portrait)` query that hides the subtitle. Reports' Create and Branches' Create render `<IconButton color="primary" size="small">` + Tooltip on mobile portrait, and the labeled MuiButton otherwise.
...
- **CreateReportDialog** (`client/src/components/report/CreateReportDialog.jsx`): MuiDialog `maxWidth="sm"` fullWidth, `fullScreen={false}` (centered on all viewports) with the shared paper treatment `sx={{ "& .MuiPaper-root": { m: 1, py: 1 } }}`; `onClose` is a no-op (prevents close on backdrop click or Escape — MUI v9 removed `disableEscapeKeyDown`); closes only via the Cancel button or a successful submit; footer buttons live in the MuiDialog `actions` slot (fixed row behind the actions divider, same pattern as the Filter Dialog) — the form is `id="create-report-form"` and Submit is `type="submit" form="create-report-form"`, so content scrolls independently inside the DialogContent 400px cap (Phase 3 corrections v3) (§12.6 3.5.1).
- **Filter Dialog (Reports):** MuiDialog `maxWidth="sm"`; title "Filter Reports"; **centered on all viewports** — explicit `fullScreen={false}` overrides the responsive-fullscreen rule, with the shared paper treatment `sx={{ "& .MuiPaper-root": { m: 1, py: 1 } }}`; DialogContent is `p: 0` with a 400px scroll cap, so the form grid restores the inset with `sx={{ px: 1, py: 2 }}` (Phase 3 corrections v3; the retired `contentSx` approach is superseded); fields stack fullwidth on portrait phones (`size={{ xs: 12, sm: 6 }}` — stacked below 600px, side by side from 600px up including mobile landscape); MuiDatePicker + MuiSelectField (single branch) with ClearIcon end adornments (clearing resets the field, decrements `activeFilterCount`, updates the badge immediately); row 2 — MuiSwitch with `labelPlacement="start"` so the "Archived" label reads left of the switch; Cancel resets filters and badge → 0; Apply sets filter state and badge → 1–3; badge hidden when 0 (§12.6 Reports).
- **MuiConfirmDialog:** used for destructive/state-changing actions (Archive/Restore/Delete) (§12.6 Reports).
- **GlobalSearchDialog:** full-screen below 600px and below 768px landscape (no border radius, 100vh); centered on larger screens — 600px wide / 80vh tall; closed by back arrow, Escape, or outside click; search input uses RHF `register('search')`, fires on Enter or click (no debounce); results grouped by entity type (Reports, Branches) in MuiAccordion sections; empty state "No results found" (§12.3).

Reusable component detail (§14 1.3, 1.9, 1.11):

- **MuiDialog** (`client/src/components/reusable/MuiDialog.jsx`) — structural wrapper that is always used instead of raw `@mui/material/Dialog`. Internal structure: `<Dialog>` (defaults + passthrough) → `<DialogTitle>` with bottom `borderBottom` divider (rendered only if `title` is provided) → `<DialogContent>` with `maxHeight: 400`, `overflowY: auto`, and `p: 0` (the only scrollable section — tall content scrolls internally inside the 400px cap instead of growing the paper; the retired `contentSx` prop is gone) → `<Divider />` (rendered only if `actions` is provided) → `<DialogActions>` (rendered only if `actions` is provided). Because DialogContent is `p: 0`, callers apply the shared paper treatment `sx={{ "& .MuiPaper-root": { m: 1, py: 1 } }}` (paper inset + vertical breathing room, e.g. the Filter, Create Report, Branch form, and Branch Selector dialogs) plus inner inset on their content (`px: 1, py: 2` on a Grid / `p: 1` on a List) (Phase 3 corrections v3). `disableEnforceFocus` and `disableRestoreFocus` default to `true` (and are always supported). Responsive fullscreen via internal `useMediaQuery` checking `theme.breakpoints.down('sm')` OR `theme.breakpoints.down('md')` with landscape — when matched, `fullScreen={true}` (no border radius, 100vh); overridable by the caller passing an explicit `fullScreen` prop (the Reports Filter Dialog, the Create Report dialog, the Branch form dialog, and the Branch Selector dialog pass `fullScreen={false}` to stay centered on all viewports — Phase 3 corrections). Callers provide MuiButton components inside the `actions` slot (e.g. `<MuiButton variant="outlined">Cancel</MuiButton>`); GlobalSearchDialog (1.11) is the exception and does not use the actions slot. Tree-shaken imports (`Dialog`, `DialogTitle`, `DialogContent`, `DialogActions`, `Divider`); `displayName` set to `"MuiDialog"`.
...
### 9. Component Catalog (§14 1.1–1.14)
...
- **Visibility (Phase 3 corrections):** the pagination row renders only when more than one page exists (`data.totalPages > 1`), i.e. when the resource count exceeds the page size (10) — a lone single-page chip is never shown.
...
- **File:** `client/src/components/reusable/MuiDataGrid.jsx`; package `@mui/x-data-grid` — community version only. Plain function component (no `forwardRef` — presentation wrapper per §14.2; no caller passes a ref). Rows are keyed via `getRowId` defaulting to `(row) => row._id` (the client's canonical identity — DM-03 strips the `id` virtual, so API docs never carry `id` and rows are never forced into an `id` field; callers may override `getRowId`; `## Redux RTK Query` §2).
- **Columns:** defined per domain in `client/src/components/columns/*.jsx`; each file exports a `build<Domain>Columns` function; the action column is the last column in every domain column set.
...
- **Export selection:** `checkboxSelection` enabled on all viewports (Phase 3 corrections — the selection column is never removed on mobile); `disableRowSelectionOnClick={true}`; export button in the toolbar for selected rows.
- **Mobile (Phase 3 corrections):** the selection column stays on every viewport; height defaults to `{ xs: 320, sm: 400 }` so the grid does not dominate small screens.
- **Toolbar (Phase 3 corrections):** `showToolbar` (default `true` in MuiDataGrid) renders MUI X v9's default toolbar — Columns toggle, Filter (with count badge), CSV export. In v9 the toolbar slot renders only when `showToolbar` is true (`GridHeader.mjs`), and the default slot is the v9 toolbar (`defaultGridSlotsComponents`); the deprecated v7 `GridToolbar` import from `@mui/x-data-grid/components` is never used (the v7 toolbar's density selector has no v9 replacement — a v9 reconciliation note; the spec's older "columns, filter, density, CSV export" line is superseded). `slotProps.toolbar` = `showQuickFilter: false` (the quick-filter field expands to 260px on focus and, on the toolbar's non-wrapping `justifyContent: end` row, pushes the buttons off narrow screens) + `sx: { flexWrap: 'wrap' }` + `csvOptions` (the v9 key — the v7-era `csvExportOptions` is not consumed by the v8 GridToolbar and leaks to the DOM) with `getRowsToExport: ({ apiRef }) => …` returning the selected row ids when a selection exists (via `apiRef.current.getSelectedRows()`), otherwise `undefined` (current view).
...
- **State coverage:** loading via the `loading` prop with skeleton `slotProps={{ loadingOverlay: { variant: 'skeleton' } }}`; empty via `slotProps={{ noRowsOverlay }}` rendering the dense MuiEmptyState variant (§9.10) — the prop is `emptyResource` (resource name, default `"rows"`; headline "No {resource} yet").
...
#### 9.10 MuiEmptyState (§14 1.14; Phase 3 corrections)

- **File:** `client/src/components/reusable/MuiEmptyState.jsx`. Reusable empty-state presentation for pages, viewport overlays, and future sections (echoes the `## UI/UX Spec` §3.6 empty states).
- **Structure:** centered flex column — icon in a soft circle (`borderRadius: '50%'`, `border: 1` divider, `bgcolor: 'background.paper'`, `color: 'text.disabled'`), headline "No {resource} yet" (`h6`, `fontWeight: 600`), optional muted subtitle (`body2` `text.secondary`), optional action element below.
- **Props:** `resource` (string, required — the headline noun: `"reports"` / `"branches"` / …), `subtitle` (string, optional), `action` (ReactNode, optional — e.g. a Create MuiButton), `icon` (ReactNode, optional — default `Inbox`), `dense` (boolean — compact variant with 40px icon circle and tighter spacing, used inside MuiDataGrid's `noRowsOverlay`), `minHeight` (default `"320px"`; dense grid usage passes `"240px"`), `sx` (Box overrides).
- **Usage:** Reports list view when the first page loads empty (`resource="reports"` + subtitle + "Create Report" action) and MuiDataGrid's `noRowsOverlay` via the `emptyResource` prop (§9.6).
- **displayName:** `"MuiEmptyState"`.
